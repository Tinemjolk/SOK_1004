...
df %>%
  ...
  ggplot(aes
         (x=tid, y=verdi, color=var,)) +
  geom_line() +
           scale_color_discrete(name= "BNP, Løpende Priser", "BNP, Faste Priser") +
  theme_minimal() +
  labs(
    title = "BNP per År",
    x = "tid",
...
Grafen viser oss at Brutto Najsonalprodukt øker, mens BNP med faste priser forblir på ett visst nivå over disse 23 årene. Grafen viser til en økt smaling av økonomisk aktivitet som øker drastisk når 2020 inntreffer. Derav kan vi tenke oss til at den økonomiske mervardeien også øker en god del. Tilstanden til økonomien ser ut som den er god, men kræsjet som kommer etter covid boblen kan påvirke resultatet.

MemoBNP-en forteller oss det reelle resultatet til BNP-en over denne tiden. Der ser vi at BNP-en holder seg stabil med faste
...
Det df_wide gjør er å endre på df sine verdier. Den erstatter df sine verdier, tid blir flyttet til begynnelsen, verdi slås sammen med BNP for å skape en egen BNP ramme, rammen med BNP fjernes siden BNP er satt sammen med verdi, derav flyttes verdien memoBNP opp for erstatte rammen BNP holdt tidligere.
...
df_wide <- df_wide %>%
  mutate(DBNP = 100 * (BNP-LBNP)/LBNP,
         DMEMOBNP = 100 * (MEMOBNP-LMEMOBNP)/LMEMOBNP)
...
# Beregne vekst (hvis ikke allerede gjort)
df_wide <- df_wide %>%
  mutate(
    vekst_nom = (BNP / LBNP - 1) * 100,
    vekst_real = (MEMOBNP / LMEMOBNP - 1) * 100  # Juster denne linjen hvis MEMOBNP ikke er reelt BNP
  )

# Visualisere
ggplot(df_wide, aes(x = tid)) +
  geom_point(aes(y = vekst_nom, color = "Nominell")) +
  geom_point(aes(y = vekst_real, color = "Reell")) +
  geom_smooth(aes(y = vekst_nom, color = "Nominell"), se = FALSE) +
  geom_smooth(aes(y = vekst_real, color = "Reell"), se = FALSE) +
  scale_color_manual(values = c("Nominell" = "purple", "Reell" = "steelblue2")) +
  labs(
    x = "År",
    y = "Prosentvis vekst",
    color = "BNP-type",
...
# Identifisere år med negativ reell vekst
negativ_vekst <- df_wide %>%
  filter(vekst_real < 0)
```

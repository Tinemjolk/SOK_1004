...
```{r}
data(mtcars) #Innebygd i R.
mtcars %>%
  ggplot(aes(cyl)) + #Variabelen vi er interessert i for histogrammet
  geom_histogram() +
  theme_minimal()#Banging graf
```
...
data(mtcars)
mtcars %>%
  ggplot(aes(mpg)) + #mpg (miles per gallon) er verdien vi er interessert i her
  geom_histogram(binwidth=0.1) +#vi deler histogrammet på 0.1 i lengde
  theme_minimal()
```
...
En god verdi for binwidth avhenger av flere faktorer, inkludert:

-   **Datamengde:** For store datasett kan flere bins være mer informative. For små datasett kan færre bins være bedre for å unngå for mye støy.

-   **Datadiskretisering:** Hvis dataene dine er diskrete (f.eks., heltall), kan det være naturlig å velge en binwidth som er lik avstanden mellom diskrete verdier.

-   **Fordeling av data:** For data som er svært skjeve eller har mange outliers, kan det være nødvendig å justere binwidth for å få en bedre visualisering.

-   **Formålet med analysen:** Avhengig av hva du ønsker å undersøke, kan forskjellige binwidths være mer hensiktsmessige.
...
df <- df %>%
  rename(næring = `næring (SN2007)`, gruppe = sysselsettingsgruppe, stat = statistikkvariabel, verdi = value)

df <- df %>%
  mutate(gruppe = str_replace(gruppe, ' sysselsatte','')) %>%
  mutate(stat = str_replace(stat, "Kostnader til innleid personale", "Innleid")) %>%
...
df_plot <- df %>%
  ...
  filter(gruppe %in% c("20-49", "50-99", "100-199", "200-499", "500<")) %>% # Filtrer for de ønskede gruppene
  filter(år %in% c(2015:2022))

# Lag histogrammet
df_plot %>%
  ggplot(aes(verdi/1000, fill = år)) +
  geom_histogram(binwidth = 1, alpha = 0.5, position = "identity") +
  facet_grid(factor(gruppe, levels = c('20-49', '50-99', '100-199', '200-499', '500<')) ~ næring) +
  scale_fill_manual(values = hcl.colors(8, "Roma")) +
  labs(title = "Fordeling av verdi for ulike næringer og sysselsettingsgrupper (2015-2022)",
       x = "Verdi (i millarder)",
       y = "Antall") +
  theme_bw()
```
...
```{r}
#Finner innstanser av FoU i datasettet
stat_navn <- df$stat %>%
  unique()
print(stat_navn)
```
...
```{r}
df_plot %>%
  ggplot(aes(verdi/1000, fill = stat)) +
  geom_histogram(binwidth = 1, alpha = 0.5, position = "identity") +
  facet_grid(factor(gruppe, levels = c('20-49', '50-99', '100-199', '200-499', '500<')) ~ næring) +
  scale_fill_manual(values = rainbow(8)) +
  labs(title = "Fordeling av verdi og FoU-bruken på kostnadstyper for ulike næringer og sysselsettingsgrupper",
       x = "Verdi",
       y = "Antall") +
  theme_bw()  # Tema for grafen
```
